...
def get_spark_session():
    ...
    """
    print(os.path.join(os.getcwd(), "sqlite-jdbc-3.34.0.jar"))
    spark = SparkSession.builder.master("local").appName("SQLite JDBC").config(
        "spark.jars",
        os.path.join(os.getcwd(), "sqlite-jdbc-3.34.0.jar")).config(
        "spark.driver.extraClassPath",
        os.path.join(os.getcwd(), "sqlite-jdbc-3.34.0.jar")).getOrCreate()
